...
* Cron任务：CronExpression，和quartz类似（但并不是使用quartz实现的）譬如 0 0/1 * * * ?
...
2. FailStore主要用于JobClient的任务提交，TaskTracker的任务反馈，TaskTracker的业务日志传输的场景下。
3. FailStore目前提供几种实现：leveldb,rocksdb,berkeleydb,mapdb,ltsdb，用户可以自由选择使用哪种,用户也可以采用SPI扩展使用自己的实现。
...
## 特性
### 1、Spring支持
LTS可以完全不用Spring框架，但是考虑到很多用户项目中都是用了Spring框架，所以LTS也提供了对Spring的支持，包括Xml和注解，引入`lts-spring.jar`即可。
### 2、业务日志记录器
...
1. 对任务队列的扩展，用户不但可以选择使用mysql或者mongo作为队列存储，也可以自己实现。
...
一般在编写TaskTracker的时候，我们只需要测试JobRunner的实现逻辑是否正确，同时又不想启动LTS进行远程测试。那么为了方便测试，LTS提供了JobRunner的快捷测试方法。实现其自己的测试集成类`com.github.ltsopensource.tasktracker.runner.JobRunnerTester`即可，并实现`initContext`和`newJobRunner`方法即可。如[lts-examples](https://github.com/ltsopensource/lts-examples)中的例子：
